  ...
  if (!listed && !module) {
    return (
      <View style={{ flex: 1, backgroundColor: "#FFFFFF", padding: 16 }}>
        <Text style={{ color: "#64748B", lineHeight: 24 }}>{copyMm.errors.notFound}</Text>
        <Link href="/academy" asChild>
          <Pressable style={{ minHeight: 44, justifyContent: "center" }}>
            <Text style={{ color: "#0F766E", fontWeight: "600" }}>{copyMm.nav.academy}</Text>
          </Pressable>
        </Link>
      </View>
    );
    ...
    >
      ...
      {canToggle ? (
        <View style={{ flexDirection: "row", gap: 8, marginTop: 16 }}>
          ...
          >
            <Text style={{ color: !mm ? "#FFFFFF" : "#001F3F", fontWeight: "600" }}>
              {copyMm.academy.languageEn}
            </Text>
          </Pressable>
          <Pressable
            onPress={() => setMm(true)}
          ...
          >
            <Text style={{ color: mm ? "#FFFFFF" : "#001F3F", fontWeight: "600", lineHeight: 22 }}>
              {copyMm.academy.languageMm}
            </Text>
          </Pressable>
        </View>
      ) : null}
      {blocks.map((block, i) => (
        <View key={`${block.type}-${i}`} style={{ marginTop: 20 }}>
          {block.title ? (
            <Text style={{ color: "#001F3F", fontWeight: "700", fontSize: 16, lineHeight }}>{block.title}</Text>
          ) : null}
          {block.content ? (
            <Text style={{ color: "#001F3F", marginTop: 8, lineHeight, fontSize: 16 }}>{block.content}</Text>
          ) : null}
        </View>
      ))}
      {quiz.length ? (
